...
export function rvvVmulToDsl(ast: AsmAst): string {
  ...
  const r0 = 1*10, r1 = 2*11, r2 = 3*12, r3 = 4*13

  return `step(s0,"选择寄存器")
step(s1,"第一步：载入与对齐")
step(s2,"第二步：位宽（128-bit = 4 × 32-bit）")
step(s3,"第三步：逐 lane 相乘：${vd}[i] = ${vs1}[i] × ${vs2}[i]")
step(s4,"第四步：写回结果")
...
text(times, 3.34, 3.5, "×", 40)
appear(tag_${vd}, ${vd}, l1, times, s3)
blink(l1, times, s3, 3, 240)

square(z0, 4.0, 3.8, "${r0}", teal)
square(z1, 5.2, 3.8, "${r1}", teal)
square(z2, 6.4, 3.8, "${r2}", teal)
square(z3, 7.6, 3.8, "${r3}", teal)
disappear(${vd}[0..3], s4)
