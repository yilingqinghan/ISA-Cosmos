...
registerUsage('rvv.vsub.vv', 'vsub.vv vd, vs2, vs1  ; 向量减法：vd[i] = vs2[i] - vs1[i]')

registerMiniDoc('rvv.vsub.vv', {
  usage: 'vsub.vv vd, vs2, vs1 ；向量减法：vd[i] = vs2[i] - vs1[i]',
  scenarios: ['向量差值计算', '距离计算', '误差计算'],
...
export function rvvVsubToDsl(ast: AsmAst): string {
  ...
  const lo = Math.min(idVd, idV2, idV1)
  const hi = Math.max(idVd, idV2, idV1)
  ...
  for (let i = 0; i < 32; i++) {
    const color =
      (i === idV2 || i === idV1 || i === idVd) ? 'teal' : 'lightgray'
    topRowRects.push(
  ...
  return `# ----------------------------------------------------
# RVV vsub.vv：4-lane (VLEN=128, SEW=32)
# 叙事：
#   s0 顶部展示 VRF 的 32 个向量寄存器，明确源: ${vs2}/${vs1}，目标: ${vd}
#   s1 载入与对齐
#   s2 位宽解释
#   s3 逐 lane 相减
#   s4 写回结果
...
step(s3,"第三步：逐 lane 相减：${vd}[i] = ${vs2}[i] - ${vs1}[i]")
step(s4,"第四步：写回结果")
...
text(minus, 3.34, 3.5, "−", 40)
appear(tag_${vd}, ${vd}, l1, minus, s3)
blink(l1, minus, s3, 3, 240)

square(z0, 4.0, 3.8, "9", teal)
square(z1, 5.2, 3.8, "9", teal)
square(z2, 6.4, 3.8, "9", teal)
square(z3, 7.6, 3.8, "9", teal)
disappear(${vd}[0..3], s4)
...
registerHandler('rvv:vsub.vv', rvvVsubToDsl)