...
registerUsage('rvv.vfadd.vv', 'vfadd.vv vd, vs2, vs1  ; 向量浮点加法：vd[i] = vs2[i] + vs1[i] (FP)')

registerMiniDoc('rvv.vfadd.vv', {
  usage: 'vfadd.vv vd, vs2, vs1 ；向量-向量浮点加法：vd[i] = vs2[i] + vs1[i]',
  scenarios: ['科学计算', '图形处理', '信号处理'],
  notes: ['遵循 IEEE 754-2008；舍入由 fcsr.frm 控制', '支持 SEW=32/64 与掩码 vm'],
...
export function rvvVfaddToDsl(ast: AsmAst): string {
  const [vdRaw, vs2Raw, vs1Raw] = ast.operands
  ...
  const v2Init = '10.5,11.5,12.5,13.5'

  return `# ----------------------------------------------------
# RVV vfadd.vv：4-lane (VLEN=128, SEW=32/64)
# 叙事：
#   s0 顶部展示 VRF 32 个寄存器，明确源: ${vs2}/${vs1}，目标: ${vd}
#   s1 载入与对齐（浮点）
#   s2 位宽提示
#   s3 逐 lane 浮点相加
#   s4 写回结果
...
step(s0,"选择寄存器")
step(s1,"第一步：载入与对齐（浮点）")
step(s2,"第二步：位宽（SEW=32 或 64）")
step(s3,"第三步：逐 lane 浮点相加：${vd}[i] = ${vs2}[i] + ${vs1}[i]")
step(s4,"第四步：写回结果")

text(vrf_title, 2.60, -0.28, "VRF（32 × 向量寄存器）", 14, #111827)
text(vrf_legend, 5.80, -0.28, "源寄存器: ${vs2}, ${vs1}    目标寄存器: ${vd}", 14, #0f172a)
...
blink(rf_v${idV2}, rf_v${idV1}, rf_v${idVd}, s0, 6, 450)

pack(${vs2}, ${vs1}, ${vd})

label(tag_${vs2}, 2.8, 1.5, "${vs2}")
label(tag_${vs1}, 2.8, 2.8, "${vs1}")
label(tag_${vd},  2.8, 4.2, "${vd}")

vec4(${vs2}, 4.0, 1.1, "${v2Init}", teal,      x, 0.2)
vec4(${vs1}, 4.0, 2.4, "${v1Init}", lightgray, x, 0.2)

disappear(vrf_title, vrf_legend, reg_row, ${Array.from({length:32},(_,i)=>`rf_v${i}`).join(', ')}, sel_box, sel_to_zoom, s1)
appear(tag_${vs2}, tag_${vs1}, ${vs2}, ${vs1}, ${vs2}__box, ${vs1}__box, s1)

label(dim, 4.8, 0.2, "SEW = 32 / 64（IEEE 754）")
appear(dim, s2)
blink(dim, s2, 4, 300)
...
square(z0, 4.0, 3.8, "11.5", teal)
square(z1, 5.2, 3.8, "13.5", teal)
square(z2, 6.4, 3.8, "15.5", teal)
square(z3, 7.6, 3.8, "17.5", teal)
disappear(${vd}[0..3], s4)
...
registerHandler('rvv:vfadd.vv', rvvVfaddToDsl)